...
class MyRunnable(Runnable):
    """
        ...
        Creates/updates:
            ...
            - One "External Model" shell per Copilot Product SKU
            - One "External Model Version" per service plan under the SKU

        Notes:
            - Uses client credentials against Microsoft Graph to list subscribed SKUs and service plans.
            - No user content is accessed; only licensing/plan metadata.
    """
    ...
    def run(self, progress_callback):
        ...
        F_PROJ_MODELS, F_MODEL_PARENT, F_MODEL_CHILD, F_VER_PARENT = (
            "govern_models",
            "govern_project",
        ...
        def _set_reference(defn, field_id: str, artifact_id: str, list_field: bool = False):
            raw = defn.get_raw(); fields = raw.setdefault("fields", {}); cur = fields.get(field_id)
            if list_field:
                if cur is None:
                ...
                if isinstance(cur, dict) and "artifactId" in cur:
                    fields[field_id] = [cur] if cur.get("artifactId") == artifact_id else [cur, {"artifactId": artifact_id}]; return
                fields[field_id] = [cur] if cur == artifact_id else [cur, artifact_id]; return
            if isinstance(cur, list):
                if artifact_id not in cur: cur.append(artifact_id)
                fields[field_id] = cur
        ...
        def upsert_model_shell(meta: dict, project_artifact_id: str | None):
            ...
            fields = {F_EXT: meta["external_id"], F_DESC: meta.get("description"), F_TAGS: json.dumps(meta.get("tags", {}))}
            fields = {k: v for k, v in fields.items() if v is not None}
            if art:
                d = art.get_definition(); raw = d.get_raw(); raw.setdefault("fields", {}).update(fields)
                if raw.get("name") != meta["name"]: raw["name"] = meta["name"]
                if project_artifact_id: _set_reference(d, F_MODEL_PARENT, project_artifact_id, list_field=False)
                d.save(); return art, "updated"
            payload = {"blueprintVersionId": GovernBlueprintVersionId(BP_MODEL_ID, BV_MODEL_ID).build(), "name": meta["name"], "fields": fields}
        ...
        def link_model_into_project(project_art, model_artifact_id: str):
            d = project_art.get_definition(); raw = d.get_raw(); fields = raw.setdefault("fields", {}); cur = fields.get(F_PROJ_MODELS)
            if cur is None:
        ...
        def upsert_model_version(meta: dict, model_artifact_id: str | None):
            ...
            fields = {
                F_EXT: meta["external_id"], F_VER: str(meta.get("version")), F_DESC: meta.get("description"),
                F_TAGS: json.dumps(meta.get("tags", {})), F_CRTON: meta.get("created_on"), F_CRTBY: meta.get("created_by"),
                F_LASTMOD: meta.get("last_modified"), F_MODBY: meta.get("modified_by"), F_PATH: meta.get("path"),
            }
            fields = {k: v for k, v in fields.items() if v is not None}
            title = f"{meta['name']} - v{meta.get('version') or '?'}"
            if art:
                d = art.get_definition(); raw = d.get_raw(); raw.setdefault("fields", {}).update(fields)
                if raw.get("name") != title: raw["name"] = title
                if model_artifact_id: _set_reference(d, F_VER_PARENT, model_artifact_id, list_field=False)
                d.save(); return art, "updated"
            payload = {"blueprintVersionId": GovernBlueprintVersionId(BP_VER_ID, BV_VER_ID).build(), "name": title, "fields": fields}
            art = gc.create_artifact(payload)
        ...
        resp = requests.post(token_url, data={"client_id": CLIENT_ID, "client_secret": CLIENT_SECRET, "scope": f"{GRAPH_BASE}/.default", "grant_type": "client_credentials"}, timeout=30)
        if not resp.ok: raise RuntimeError(f"Graph token error: {resp.status_code} {resp.text}")
        access_token = resp.json().get("access_token")
        if not access_token: raise RuntimeError("No access_token from Microsoft identity platform.")

        # Clients
        gc = dataikuapi.GovernClient(GOVERN_BASE.rstrip("/"), GOVERN_APIKEY, insecure_tls=True)
        api_ver = "beta" if GRAPH_USE_BETA else "v1.0"; headers = {"Authorization": f"Bearer {access_token}"}

        # Org
        org = requests.get(f"{GRAPH_BASE}/{api_ver}/organization", headers=headers, timeout=30); org.raise_for_status()
        org_value = (org.json() or {}).get("value", []); tenant_display_name = (org_value[0] or {}).get("displayName") if org_value else TENANT_ID
        project_art, _ = upsert_project(tenant_display_name)

        # SKUs
        skus = requests.get(f"{GRAPH_BASE}/{api_ver}/subscribedSkus", headers=headers, timeout=60); skus.raise_for_status()
        sku_items = (skus.json() or {}).get("value", [])

        count_models, count_versions = 0, 0
        for sku in sku_items:
            sku_id = sku.get("skuId"); sku_part = sku.get("skuPartNumber"); service_plans = sku.get("servicePlans", []) or []
            if not sku_part or "COPILOT" not in sku_part.upper():
                continue
            model_meta = {"external_id": f"m365:sku:{sku_id}", "name": f"Microsoft Copilot - {sku_part}", "description": "Microsoft Copilot SKU via Microsoft Graph", "tags": {"cap_total_units": sku.get("capabilityStatus"), "prepaid_units": json.dumps(sku.get("prepaidUnits", {})), "consumed_units": sku.get("consumedUnits")}}
            model_art, _ = upsert_model_shell(model_meta, project_artifact_id=project_art.artifact_id); link_model_into_project(project_art, model_art.artifact_id); count_models += 1
            for sp in service_plans:
                sp_name = sp.get("servicePlanName");
                if ONLY_PLAN_NAME and sp_name != ONLY_PLAN_NAME: continue
                ver = sp.get("servicePlanId"); state = sp.get("provisioningStatus"); applies_to = sp.get("appliesTo")
                ver_meta = {"external_id": f"m365:sku:{sku_id}:plan:{ver}", "name": sku_part, "version": ver, "description": f"Service plan {sp_name} (state={state}, appliesTo={applies_to})", "tags": {"servicePlanName": sp_name, "provisioningStatus": state, "appliesTo": applies_to}, "created_on": None, "created_by": None, "last_modified": datetime.now(timezone.utc).isoformat(), "modified_by": "Microsoft Graph", "path": None}
                _ = upsert_model_version(ver_meta, model_artifact_id=model_art.artifact_id); count_versions += 1
